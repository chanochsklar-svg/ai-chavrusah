...
def generate_elevenlabs_speech(text, voice_id=None):
    # Deliberately ElevenLabs, not the browser's built-in speechSynthesis API —
    # browser TTS voices are synthetic-sounding and can't be fixed by picking a
    # different one; ElevenLabs' multilingual model is what actually sounds
    # human and reads Hebrew script correctly.
    if not ELEVENLABS_ENABLED:
        return None
...
Pronunciation matters because your replies are read aloud by a text-to-speech voice: whenever you use a
Hebrew or Aramaic word, name, or technical term — even in an otherwise-English reply — write it in its
original Hebrew script (e.g. סוכה, רש"י, גמרא) rather than a Romanized spelling like "Sukkah" or "Rashi".
The voice engine reads Hebrew script accurately but often mispronounces Romanized transliterations. You may
add an English gloss in parentheses the first time a term appears if it aids clarity.
...
def steering_reminder(segment_ref: str, en_text: str, he_text: str, is_new_segment: bool) -> str:
    ...
    language_note = (
        "Reply in whichever language — Hebrew or English — the user's last message was written in. "
        "Write any Hebrew/Aramaic terms in Hebrew script, not Romanized spelling, since your reply is spoken aloud."
    )
